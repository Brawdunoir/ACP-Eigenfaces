...
!--------------------------------------------------------------------------------------------------------------------
  subroutine subspace_iter_v0(n, m, a, maxit, eps, v, w, acc, it, ierr)
    ...
    do while((acc .ge. eps) .and. (k .lt. maxit))
      ...
      call dgemm('n', 'n', n, m, m, done, v, n, h, m, dzero, aux, n)
      aux = y - aux

        !! Compute acc = ||aux|| / ||A||

      normF_aux = dlange('f',n,m,aux,n,work)
      acc = normF_aux / normF_A
      

      acc = sqrt(acc)/normF_A
    ...
    if(acc .lt. eps) then
      ...
      !! v = v*x
      call dgemm('n', 'n', n, m, m, done, v, n, x, m, dzero, v, n) 

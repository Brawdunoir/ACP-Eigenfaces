...
normale = makedist('Normal');

x = normrnd(0,1,[2,nb_indiv]);
x_mean = [sum(x(1,:))/nb_indiv,sum(x(2,:))/nb_indiv];
...
title('des nombres sur le plan des reels');
...
x = normrnd(2,3,[3,nb_indiv]); 
x_mean = [sum(x(1,:))/nb_indiv,sum(x(2,:))/nb_indiv,sum(x(3,:))/nb_indiv];
...
legend('individus','element moyen');
title("des nombres sur l'espace des reels");
